...
const Navbar = () => {
  ...
  return (
      ...
      <div
        className="flex justify-between items-center overflow-scroll w-screen gap-8 mt-5 p-2 overflow-y-hidden "

        style={{scrollbarColor: "#D17842 black",scrollbarWidth: "thin",cursor: "pointer"}}
      >
        {navbarItem.map((item, index) => {
          return (
            <div className="flex flex-col items-center self-start text-sm text-nowrap" key={index}>
              <Link href={`/${item.englishTitle}`}
                className={
